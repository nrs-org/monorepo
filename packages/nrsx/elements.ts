...
import { assert, ScalarMatrix, makeEntryMeta } from "@nrs-org/core";
...
import type {
  ...
  ProgressStatus,
} from "@nrs-org/ext-dah-entry-progress";
import type { ExtDAH_entry_title } from "@nrs-org/ext-dah-entry-title";
import type { ExtDAH_entry_type, EntryType } from "@nrs-org/ext-dah-entry-type";
...
export function Entry(props: EntryProps): EntryNode {
  return asEntry((rc) => {
    ...
    // Set title via extension if available.
    const titleExt = rc.context.extensions["DAH_entry_title"] as
      | ExtDAH_entry_title
      | undefined;
    if (titleExt) {
    ...
    // Set entry type via extension if available.
    if (props.entrytype !== undefined) {
      const typeExt = rc.context.extensions["DAH_entry_type"] as
        | ExtDAH_entry_type
        | undefined;
      if (typeExt) {
 ...
 */
export function ValidatorSuppress(props: ValidatorSuppressProps): ImpactNode {
  return asImpact((rc) => {
